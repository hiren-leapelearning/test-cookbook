node[:deploy].each do |application, deploy|
  ...
  template "#{node[:deploy][application][:deploy_to]}/current/application_lms/config/zoho.php" do
    source "#{node[:deploy][application][:deploy_to]}/utilities/deploy-files/lms/#{node[:environment]}/zoho.php"
  end
  template "#{node[:deploy][application][:deploy_to]}/current/application_lms/config/xero.php" do
    source "#{node[:deploy][application][:deploy_to]}/utilities/deploy-files/lms/#{node[:environment]}/xero.php"
  end
